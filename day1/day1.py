...
def parse_dates(s: str) -> Optional[date]:
    s = (s or "").strip()
    if not s:
        return None
...
def urgency_score(days_left: Optional[int]) -> float:
    if days_left is None:  ## no tasks
        return 0.5
    if days_left < 0:  ## overdue
        return 5.0
    if days_left == 0:  ## due today
        return 5.0
    if days_left == 1:
        return 4.0
    if days_left <= 3:
...
def compute_score(task: Task) -> Tuple[float, Dict[str, float]]:
    ...
    bpen = WEIGHTS["blocked_penality"] if task.blocked else 0.0
...
def reason(task: Task, breakdown: Dict[str, float]) -> float:
    reasons = []
    dleft = days_until(task.deadline)
    ...
    if task.effort_min <= 15:
        reasons.append("Quickwin")

    if task.blocked:
        reasons.append("Blocked(needs unlock step)")
    return ",".join(reasons)
...
def read_tasks(path: str) -> List[Task]:
    ...
    with open(path, "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            deadline = parse_dates(row.get("deaddline") or "")
            effort_min = parse_efforts(row.get("effort") or "")
...
def build_plan(tasks: List[Task]) -> Dict:
    ...
    blocked = [(t, s, b) for (t, s, b) in scored if not t.blocked]
...
def render_summary(plan: Dict) -> str:
    ...
    def section(name: str, items: List[Dict]):
        ...
        for i, it in enumerate(items, 1):
            ...
            lines.append(
                f"{i}.{it['title']} | deadline:{dl} | effort :{it['effort_min']}m | score : {it['score']}"
            )
...
def main():
    tasks = read_tasks(r"C:\Users\HP\Python\Projects\Agents\tasks.csv")
    if not tasks:
...
###  prepare task.csv file in proper format
