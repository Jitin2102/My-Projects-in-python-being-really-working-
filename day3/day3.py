...
def read_calendar(
    path=r"C:\Users\HP\Python\Projects\Agents\calendar.csv",
) -> List[Event]:
    events = []
...
def suggest_resolution(a: Event, b: Event) -> str:
    ...
    if a.priority < b.priority and a.flexible:
...
def main():
    ...
    with open("conflicts.txt", "w", encoding="utf-8") as f:
        ...
        for c in conflicts:
            f.write(f"-Conflicts between {c['event_a']} and {c['event_b']}\n")
            f.write(f"-Type:{c['type']}, Severity :{c['severity']}\n")
            f.write(f"Suggested Action :{c['suggestion']}\n\n")
        print("conflict anlysis complete.")
